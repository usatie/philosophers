...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/10/23 11:11:43 by susami            #+#    #+#             */
/*   Updated: 2022/10/23 22:25:55 by susami           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	unsafe_log_action(t_philo *philo, const char *msg, t_timeval *tp)
{
	...
	error = 0;
	sem_wait_exit_on_err(philo->e->monitor.sem);
	if (!philo->e->monitor.is_dead)
	{
		...
		error = -1;
	sem_post_exit_on_err(philo->e->monitor.sem);
	return (error);
}
